...
# Azure Speech Translation setup
speech_key = "a446630e73514d779093ab5621f15304"
service_region = "eastus"
 
def translate_text(text, from_language="en", to_language="fr"):
    # Initialize the speech translation configuration
    ...
    # Translate text
    result = translator.recognize_once_from_text(text)
...
async def transcription(sid, data):
    ...
    # Translate the transcription using Azure Speech Translation
    translated_text = translate_text(transcription)
 
    # Send the translated text back to the client
    await sio.emit('translatedText', {'username': username, 'translatedText': translated_text}, room=sid)
    print(f'Transcription from {username} in room {room_code}: {transcription}')
    print(f'Translated transcription: {translated_text}')
